...
if [ $np_all -lt $(($maxnumbertosubmit + 3)) ]
then
...
do
   ...
   do
       ...
       do
         ...
         echo "n $n *****************************"
         ens=`printf '%.3d' $n`
         export caso=${SPSSystem}_${yyyy}${st}_${ens}
set +euvx
         . ~/load_miniconda; conda activate env_tools
set -euvx
         n_done_on_juno=`python $DIR_CPS/csv_reader.py`
set +euvx
         conda deactivate
set -euvx
  
         if [[ $n_done_on_juno -eq 1 ]] 
         then
            continue
         fi
         # is running?
