...
sed -e "s/YYYY/${yyyy}/g;s/STDATE/$st/g;s/PATM/$pp/g;s/PLAND/$ppland/g;s/POCE/$poce/g;s/NRUN/$nrun/g;" $DIR_CPS/create_caso.sh > $SCRATCHDIR/cases_$st/create_caso_leonardo_$caso.sh
chmod u+x $SCRATCHDIR/cases_$st/create_caso_leonardo_$caso.sh
$SCRATCHDIR/cases_$st/create_caso_leonardo_$caso.sh
