#!/bin/sh -l
# THIS MUT BE RUN ON COMPUTE-NODE
# once the executable has been produced check that all the dependencies are resolved with ldd cesm.exe
. ~/.bashrc
...
mkdir -p $DIR_REFCASES
if [[ -d $DIR_REFCASES/$caso ]]
then
   rm -rf $DIR_REFCASES/$caso
fi

# activate the conda env needed by CIME
...
$DIR_CESM/cime/scripts/create_newcase --case $DIR_REFCASES/$caso --compset HIST_CAM60%WCSC_CLM51%BGC-CROP_CICE_NEMO_HYDROS_SGLC_SWAV --res f05_n0253 --driver nuopc --mach $machine --run-unsupported

cd $DIR_REFCASES/$caso
...
if [[ $machine == "leonardo" ]] ; then
   rsync -av $DIR_TEMPL/env_mach_specific.xml_${env_workflow_tag} $DIR_REFCASES/$caso/env_mach_specific.xml
fi
./case.setup
