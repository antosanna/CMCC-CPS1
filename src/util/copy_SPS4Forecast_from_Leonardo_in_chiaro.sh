#!/bin/sh -l
#BSUB -J copy_SPS4Forecast_from_Leonardo_in_chiaro
#BSUB -q s_download
#BSUB -o /work/cmcc/cp1/CPS/CMCC-CPS1/logs/leonardo_transfer/copy_SPS4Forecast_from_Leonardo_in_chiaro.out.%J  
#BSUB -e /work/cmcc/cp1/CPS/CMCC-CPS1/logs/leonardo_transfer/copy_SPS4Forecast_from_Leonardo_in_chiaro.err.%J  
#BSUB -P 0784
#BSUB -M 1000

. $HOME/.bashrc
. $DIR_UTIL/descr_CPS.sh
# First check that no other this script is running
#load module for sshpass
module load $modulepass
set -uvx

yyyy=`date +%Y`
st=`date +%m`

if [[ $machine != "cassandra" ]]
then
   exit
fi
DIR_ARCHIVE=$DIR_ARCHIVE/Leonardo_${yyyy}${st}
mkdir -p $DIR_ARCHIVE

job_run=`$DIR_UTIL/findjobs.sh -m $machine -n copy_SPS4Forecast_from_Leonardo_in_chiaro -c yes`
if [[ $job_run -gt 1 ]]
then
   exit 0
fi

yyyy=`date +%Y`
st=`date +%m`
leo_dir=/leonardo_work/$account_SLURM/CMCC-CM/archive #/leonardo_work//CMCC-CM/archive/
leo_dir_CASES=/leonardo_work/$account_SLURM/CPS/CMCC-CPS1/cases #/leonardo_work///CPS/CMCC-CPS1/cases
leo_dir_temp=/leonardo_work/$account_SLURM/scratch/CMCC-CPS1/temporary
leo_dir_REP=/leonardo_work/$account_SLURM/CPS/CMCC-CPS1/logs/REPORTS/$yyyy$st
#if submit_tarC3S started, stop the copy from Leonardo to avoid issues with renumbering
if [[ -f ${DIR_LOG}/forecast/${yyyy}${st}/submit_tar_C3S_${yyyy}${st}_started ]] ; then
   echo "submit_tarC3S started, no extra cases to be copied. Exiting now."
   exit 0
fi

# get the list of completed cases (produced daily in cron on Leonardo)
outdir=${SCRATCHDIR}/Leonardo_transfer_${yyyy}${st}
mkdir -p $outdir
lista_today_1=" "
for ens in {001..009} ; do
   caso=sps4_${yyyy}${st}_${ens}

   checkfile=$DIR_ARCHIVE/$caso.transfer_from_Leonardo_DONE
   rsync -auv -e="sshpass -p a(nU05wgJk ssh" a07cmc00@data.leonardo.cineca.it:${leo_dir}/$caso $DIR_ARCHIVE
   stat=$?
   if [[ $stat -eq 0 ]]
   then
      chmod -R ug-w ${DIR_ARCHIVE}/$caso
      touch $checkfile
      dim=`du -hs $DIR_ARCHIVE/$caso|cut -c 1-3`
   else
      continue
   fi

done
exit
repfile=REPORT_leonardo.sps4_${yyyy}$st.txt
mkdir -p $DIR_REP/$yyyy$st
rsync -auv --rsh="sshpass -f $HOME/.sshpasswd ssh -l a07cmc00" a07cmc00@data.leonardo.cineca.it:$leo_dir_REP/$repfile $DIR_REP/$yyyy$st
