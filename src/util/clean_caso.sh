...
if [ -d $DIR_ARCHIVE_C3S/$startdate ] ; then
         ...
         if [ $(ls -1 cmcc_CMCC-CM3-v${versionSPS}_${typeofrun}_S${startdate}0100*r${ens}i00p00.nc | wc -l ) -gt 0 ]; then
               rm -f cmcc_CMCC-CM3-v${versionSPS}_${typeofrun}_S${startdate}0100*r${ens}i00p00.nc
         fi
         # if exist some c3s logs files remove them (c3s logs with ok)
         if [ $(ls -1 *r${ens}i00p00_ok | wc -l ) -gt 0 ]; then
            rm -f *r${ens}i00p00_ok
         fi
         # if exist some c3s logs files remove them (qa,meta,tmpl logs)
         if [ $(ls -1 *_ok_0${ens} | wc -l ) -gt 0 ]; then
            rm -f *_ok_0${ens}
         fi
