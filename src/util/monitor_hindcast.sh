...
function write_help
{
  echo "Use: monitor_hindcast.sh [<st>] [<do plots:1=yes;0=no>]"
}
...
if [ $2 -eq 0 ]  ; then # manage the case ./monitor_forecast
    doplot=0
elif [ $2 -eq 1 ]  ; then # manage the case ./monitor_forecast 1
    doplot=1
fi
...
do
   start_date=${yyyy}${st}
   # `date +%Y%m`
   if [[ `ls -d $WORK_CPS/${SPSSystem}_${start_date}_0??/run |wc -l` -eq 0 ]] ; then 
         continue 
   fi
