#!/bin/sh -l
#BSUB -J copy_SPS4DMO_from_Leonardo5
#BSUB -q s_download
#BSUB -o /work/cmcc/cp1/CPS/CMCC-CPS1/logs/leonardo_transfer/copy_SPS4DMO_from_Leonardo5.out.%J  
#BSUB -e /work/cmcc/cp1/CPS/CMCC-CPS1/logs/leonardo_transfer/copy_SPS4DMO_from_Leonardo5.err.%J  
#BSUB -P 0490
#BSUB -M 1000
...
rsync -auv --rsh="sshpass -f $HOME/.sshpasswd ssh -l a07cmc00" a07cmc00@dmover3.leonardo.cineca.it:/leonardo_work/$account_SLURM/scratch/list_`date +%Y%m%d` $DIR_TEMP


st=12
lista_today_5=" "
for ens in `seq -w 011 020` ; do
   caso=sps4_2024${st}_${ens}
...
idjob=`$DIR_UTIL/findjobs.sh -n copy_SPS4DMO_from_Leonardo5 -i yes`
logfile=$DIR_TEMP/list5_cases_transferred_`date +%Y%m%d`.$idjob.txt
echo $lista_today_5 > $logfile
