...
# all vars at once
$DIR_DIAG/prompt_launch_remap_var_diag_runtime.sh $yyyy $st $nrun $scriptdir $nmf $WKDIR $inputfile $dbg "$invarlist"
echo "fatto?bene!"
while `true`
do
   n_outputC3S=`ls $WKDIR/${SPSSystem}*.reg1x1.*.nc|wc -l`
   if [[ $n_outputC3S -ge $(($nrun * $n_vars)) ]]
   then
      break
   fi
   sleep 30
...
#   rm -rf $WKDIR/*_${yyyy}${st}_*
   ${DIR_DIAG}/anom_${CPSSYS}_runtime.sh $yyyy $st $refperiod $var $nrun $WKDIR $flgmnth $monthstr $nmf $climdir $pctldir $pctlvar $colormap $units $unitsl $fact $pldir $dbg
done
