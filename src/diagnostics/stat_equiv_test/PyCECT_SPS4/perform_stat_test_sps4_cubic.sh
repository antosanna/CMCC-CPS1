...
TESTARCHIVE=/work/cmcc/cp1/scratch/check_equivalence_remapping/
mkdir -p $TESTARCHIVE


# !WARNING HERE !WARNING HERE !WARNING HERE
merge_ens=1 # activate ensemble cat

# !WARNING HERE !WARNING HERE !WARNING HERE
merge_test=1 # activate ensamble cat
...
testname="${SPSSystem}_${st}_test${members}ens_cubicVSppm"
...
startdate2=202105
listofcases="13 17 20 21 22 23 24 25 26 27 29"
...
if [ $merge_ens -eq 1 ]; then
  ...
  for yyyy in $(seq -w 1993 2022)  ; do
	 
       count=0
       for pp in $(seq -w 001 030); do 
           
           caso=${SPSSystem}_${yyyy}${st}_${pp}
           if [[ ! -f $DIR_CASES1/$caso/logs/${caso}_6months_done ]] &&  [[ `ls $DIR_ARCHIVE1/$caso.transfer_from_*_DONE|wc -l` -eq 0 ]] ; then
       		    continue
           fi
       	   filelisttomerge=()
