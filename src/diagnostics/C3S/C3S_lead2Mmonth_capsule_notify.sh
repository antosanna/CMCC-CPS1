...
for ff in $flist ; do
  ...
  pp=`basename $ff | cut -d '_' -f9 | cut -d '.' -f1 | cut -c2-3`
	 ...
	 sps="sps_${yyyy}${st}_${ppp}"
  finaloutput=${var}_${SPSSYS}_${sps}.nc

	 if [ ! -f ${DIR_LOG}/${typeofrun}/$yyyy$st/diagnostics/capsule_${yyyy}${st}_${ppp}_${var}_DONE ] ; then
		# GET Data
		#rsync -auv $datadir/*${varC3S}_*r${pp}i00p00.nc .
 		  cd $datadir 
		   tfile=`ls -1 cmcc_CMCC-CM2-v*_*_S${yyyy}${st}0100_*_${varC3S}_r${pp}i00p00.nc`
	   	cd $workdir
	   	ppp=`printf "%03d" $(( 10#${pp} ))` 
		   sps="sps_${yyyy}${st}_${ppp}"
     ...
     touch ${DIR_LOG}/${typeofrun}/$yyyy$st/diagnostics/capsule_${yyyy}${st}_${ppp}_${var}_DONE
