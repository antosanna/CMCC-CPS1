...
refcase="cm3_lndHIST_bgc_NoSnAg_eda${member}_hist"
caze="cm3_lndSSP5-8.5_bgc_NoSnAg_eda${member}_scen"
if [[ -d $DIR_CASES/$caze ]] ; then
  rm -rf $DIR_CASES/$caze
fi
if [[ -d /work/$DIVISION/$USER/CMCC-CM/${caze} ]] ; then
   rm -rf /work/$DIVISION/$USER/CMCC-CM/${caze}
fi
...
sed 's/MEMBER/'$member'/g' ${DIR_TEMPL}/user_nl_datm_streams_eda_scen > $DIR_CASES/$caze/user_nl_datm_streams

#to deactivate reset_snow (we want to keep snow from historical - needed for spinup when switching from GSWP to EDA)
...
#to-be-revised
./xmlchange RUN_REFDIR="/work/csp/cp1/CMCC-CM/archive/${refcase}/rest/2015-01-01-00000" 
