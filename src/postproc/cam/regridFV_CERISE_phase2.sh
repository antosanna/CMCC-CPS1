...
export C3Stable="$DIR_POST/cam/CERISE_table.txt"
export C3Satts="$DIR_TEMPL/CERISE_globalatt.txt"
export GCM_and_version=${GCM_name}-v${version}
export ini_term=cmcc_CERISE-${GCM_and_version}_${typeofrun}_S${yyyy}${st}0100
...
mkdir -p $SCRATCHDIR/regrid_CERISE_phase2/$caso/CAM
if [[ $type == "h3" ]]
then
   ...
   then
       ...
       export C3Stable="$wkdir/CERISE_table_noSOLIN.txt"
       #remove last line of C3Stable - which MUST be SOLIN
       sed '$ d' $DIR_POST/cam/CERISE_table.txt > $C3Stable
       ...
       if [[ ${solinfile_n} -ne 0 ]] ; then
             ...
             rsync -auv $solinfile_templ $SCRATCHDIR/regrid_CERISE_phase2/$caso/CAM/
             ...
             ncap2 -Oh -s 'realization(0:8)="r'$member'i00p00"' $SCRATCHDIR/regrid_CERISE_phase2/$caso/CAM/$solinfile_templ_name $SCRATCHDIR/regrid_CERISE_phase2/$caso/CAM/${solinfile_new_name}
             ncatted -Oh -a ic,global,o,c,"$ic" $SCRATCHDIR/regrid_CERISE_phase2/$caso/CAM/${solinfile_new_name}
             rsync -auv $SCRATCHDIR/regrid_CERISE_phase2/$caso/CAM/${solinfile_new_name} $outdirC3S
       else
             echo "NO SOLIN file to be used as template for case $caso, which does not have SOLIN ouput in $type cam output file."
             body="NO SOLIN file to be used as template for case $caso, which does not have SOLIN ouput in $type cam output file. Exiting now. When at least one member in $outdirC3S will have completed SOLIN postproc, delete $DIR_TEMP/CERISE_postproc_offline_${caso} to allow automatic resubmission. "
             title="[CERISE] ${CPSSYS} forecast warning "
             ${DIR_UTIL}/sendmail.sh -m $machine -e $mymail -M "$body" -t "$title" -r "$typeofrun" -s $yyyy$st
             if [[ -f ${DIR_TEMP}/CERISE_postproc_offline_${caso} ]] 
             then
                  #kill the launcher to allow for new submission
                  postproc_C3Sid=`${DIR_UTIL}/findjobs.sh -m $machine -n postproc_CERISE_offline_${caso} -i yes`
                  set +e
...
if [ ! -f ${checkfile} ] 
then
   cp $DIR_POST/cam/regridFV_CERISE_template.ncl $wkdir/regridFV_CERISE.$type.ncl
   sed -i "s/TYPEIN/$type/g;s/MEMBER/$real/g;s/FRQIN/$frq/g" $wkdir/regridFV_CERISE.$type.ncl
   ncl $wkdir/regridFV_CERISE.$type.ncl
fi
if [ -f ${checkfile} ]
then
   echo "regridFV_CERISE.ncl completed successfully for $type and $real"
else
# if check file does not exist send ERROR email
   touch ${check_regridC3S_type}_${type}_ERROR
   body="regridFV_CERISE.ncl anomalously exited for start-date ${yyyy}${st}, file type $type and member $real "
