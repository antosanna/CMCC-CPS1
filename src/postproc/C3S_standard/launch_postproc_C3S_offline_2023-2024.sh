...
recover=${1:-0} #default recover=0 (do not relaunch cases with missing all_checkers_ok)
...
do
  if [[ ${recover} -eq 0 ]] ; then
     ...
     if [[ ${cnt_subm} -eq ${ntot} ]] ; then
         cnt_all_ok=`ls ${WORK_C3S}/????${st}/all_checkers_ok_0??|grep -v 2025|wc -l`
         if [[ ${cnt_all_ok} -eq ${cnt_subm} ]] ;then
          #    echo "C3S postproc completed for startdate ${st}. Exitinig now." 
          #    rm ${flag_running}
          #   exit 0

              echo "C3S postproc completed for startdate ${st}. Going on with the following one."
              continue 
         else
