...
then
   ...
   for ft in $filetyp ; do
       ...
       ${DIR_UTIL}/submitcommand.sh -m $machine -q $parallelq_l -M ${req_mem} -p create_clm_files_${ft}_${caso} -S qos_resv -j postpc_clm_${ft}_${caso} -l $dir_cases/$caso/logs/ -d ${DIR_POST}/clm -s postpc_clm_CERISE_mrlsl.sh -i "$input"
   done
fi
