...
lista=`ls $WORK_C3S1/$yyyy$st/${orig_term}*r${mem}i00p00.nc| grep -v ocean_mon|grep -v seaIce_6hr_surface_sitemptop|grep -v 100m_|grep -v prw|grep -v tsl_|grep -v lweprc_|grep -v 6hr_surface_clt|grep -v day_soil_mrlsl`
for ff in $lista
do
   ...
   ncatted -O -h -a source,global,o,c,"cerise-CMCC-CM3-v20231101:  atmos: CAM(fv_0.47x0.63 L83); ocean: NEMOv4.2 (ORCA0.25_z75, 0.25x0.25L75); land: CLM(fv_0.47x0.63 L25); seaice: CICE(same horizontal resolution of NEMO)" $WORK_CERISE/$yyyy$st/$newff
   if [[ $ff =~ "sftlf" ]]
   then
