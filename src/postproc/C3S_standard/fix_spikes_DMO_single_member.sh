...
then
   ...
   #HEALING WORKING FINE ONLY on CAM OUTPUT!!
   for model in cam
   do
      case $model in
          cam) ftlist="h1 h2 h4";dir=atm;;
      esac
      for ftype in $ftlist
      do
...
do
   ...
   $DIR_C3S/poisson_daily_values.sh h3 cam $caso $inputascii $inputFV $outputFV $checkfile
...
do
   case $model in
       cam) ftlist="h1 h2 h3 h4";dir=atm;;
   esac
   for ftype in $ftlist
   do
      file2check=${caso}.$model.$ftype.${yyyy}-${st}.zip.nc
      ...
      ${DIR_UTIL}/submitcommand.sh -m $machine -q $serialq_m -M 30000 -d ${DIR_C3S} -j poisson_daily_values_${model}.${ftype}_${caso} -s poisson_daily_values.sh -l $logdir -i "$ftype $model $caso $inputascii_all $inputFV $outputFV $checkfile"
