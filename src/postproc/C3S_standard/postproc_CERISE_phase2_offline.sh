...
then
   ...
   for ft in $filetyp ; do
       ...
       ${DIR_UTIL}/submitcommand.sh -m $machine -q $parallelq_l -M ${req_mem} -p create_clm_files_${ft}_${caso} -S qos_resv -j postpc_clm_${ft}_${caso} -l $dir_cases/$caso/logs/ -d ${DIR_POST}/clm -s postpc_clm_CERISE.sh -i "$input"
   done
fi
...
#   while [[ ! -f ${check_merge_cam_files}_h1 ]] || [[ ! -f ${check_merge_cam_files}_h2 ]] || [[ ! -f ${check_merge_cam_files}_h3 ]]
   ...
   do
       ...
       then
          ${DIR_UTIL}/sendmail.sh -m $machine -e $mymail -M "$jobid create_cam_files exited" -t "[CERISE_phase2] ERROR: ${caso} create cam exited" 
          exit 1
       fi
   done
