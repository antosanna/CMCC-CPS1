...
#
cd $dirout
if [[ ! -f $dirout/mask.$yyyy$st.925hPa.4attempt.nc ]]
then
   if [[ -f $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc.gz    ]]
   then
      gunzip $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc.gz
   fi
   cdo -setrtoc2,0,85500,0,1 $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc $dirout/mask.$yyyy$st.925hPa.4attempt.nc
fi
if [[ ! -f $dirout/mask.$yyyy$st.1000hPa.4attempt.nc ]]
then
   if [[ -f $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc.gz    ]]
   then
      gunzip $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc.gz
   fi
   cdo -setrtoc2,0,85500,0,1 $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc $dirout/mask.$yyyy$st.1000hPa.4attempt.nc
fi
if [[ -f $outdirmax/PS.sps4_${yyyy}${st}.max.C3S.nc ]]
then
