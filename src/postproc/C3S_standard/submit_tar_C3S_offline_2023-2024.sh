...
# MA MAI TESTATO QUI!!!! -
        ...
        then 
           ...
           $DIR_UTIL/submitcommand.sh -m $machine -q $serialq_l -M 5000 -j tar_C3S_${startdate} -l $DIR_LOG/$typeofrun/$startdate/ -d ${DIR_C3S} -s tar_C3S_2023-2024.sh -i "$input"
        fi    
     else
