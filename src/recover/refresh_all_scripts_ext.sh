...
sed -e "s/CASO/$caso/g;s/YYYY/$yyyy/g;s/mese/$st/g" $DIR_TEMPL/check_10ext_months_output_in_archive.sh > $DIR_CASES/$caso/check_10ext_months_output_in_archive_${caso}.sh
...
sed -e "s:CASO:$caso:g;s:IC:$ic:g;s:OUTDIRC3S:$outdirC3SEXT:g" $DIR_POST/cice/interp_cice2C3Sext_template.sh > $DIR_CASES/$caso/interp_cice2C3Sext_${caso}.sh
