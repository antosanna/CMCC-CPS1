...
{
  echo "USE ONLY FROM crontab if not dbg: recover_interrupted.sh -d [<dbg>] -m [<st>] -y [<yyyy>] -e [<1/0>]"
...
function write_help
{
  echo "Use: recover_interrupted.sh -d [<dbg>] -m [<st>] -y [<yyyy>] -e [<1/0>]"
  echo "     (if hindcast only -m st else also -y yyyy)"
  echo ""
  echo "SUBMISSION COMMAND:"
  echo ". $HOME/.bashrc && . ${DIR_UTIL}/descr_CPS.sh && ${DIR_UTIL}/submitcommand.sh -m $machine -q $serialq_m -j recover_interrupted_${stmain}_debug$dbg -l $DIR_LOG/hindcast/ -d $DIR_RECOVER -s recover_interrupted.sh -i \"-d \$dbg -m \$stmain\""
  echo ""
  echo "CAVEAT"
  echo "---First time dbg should be set to 2 to onyl print the list of interrupted jobs; then 1 to postprocess only one; 0 to process all the list"
  echo "---If you want to recover a specific list of cases hardcoded in the script just enter one argument (dbg)"
...
elif [[ $machine == "juno" ]] || [[ $machine == "cassandra" ]]
then
  njobs_max=1
...
filecsv=$DIR_LOG/$typeofrun/${header}_${typeofrun}_recover${dbg}_list.${machine}.`date +%Y%m%d%H`.csv
...
#compute num of months run nmonthsrun
           nmonthsrun=$(( 10#$((cyy - yyyy)) * 12 + 10#$cmm - 10#$st ))
           ...
           if [[ $nmonthsrun -lt $nmonths_to_run ]] 
           then
...
if [[ $dbg -eq 2 ]] && [[ $USER == $operational_user ]]
then
...
if [[ $dbg -ne 2 ]] ; then
   ...
   then
      ...
      if [[ $typeofrun == "forecast" ]] && [[ $USER == $operational_user ]]
      then
...
# lt_archive
   ...
   for caso in $lista_moredays
   do
      $DIR_RECOVER/refresh_all_scripts.sh $caso $extended
